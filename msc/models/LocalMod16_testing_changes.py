...
def LEwetfun_day(df, gl_sh, gl_e_wv):
    ...
    df['LEwet_day'] = (
                              df.s_day * df.Ac_day + df.rho_day * Cp * df.vpd_day * df.Fc * df.daylength / df.rhrc) * df.Fwet_day / (
...
def mSM(df, sm_open, sm_close):
    df['msm'] = (df['fSM-rz'] - sm_close) / (sm_open - sm_close)
...
def LEtrans_day(df, Cl, gl_sh, vpd_open, vpd_close, tmin_open, tmin_close, sm_open, sm_close):
    ...
    dat['rs'] = 1.0 / dat.Cc
    dat['rh'] = 1.0 / gl_sh
    dat['rr'] = dat.rho_day * Cp / (
            4.0 * SBC * dat.ta_day * dat.ta_day * dat.ta_day)  # (dat.ta_day+273.15)*(dat.ta_day+273.15)*(dat.ta_day+273.15)
    dat['ra'] = (dat.rh * dat.rr) / (dat.rh + dat.rr)
    dat['LEtrans'] = (dat.s_day * dat.Ac_day + dat.rho_day * Cp * dat.vpd_day * dat.Fc * dat.daylength / dat.ra) * (
            1. - dat.Fwet_day) / (dat.s_day + dat.gama_day * (1 + dat.rs / dat.ra))
...
def LEtrans_night(df, gl_sh):
    ...
    dat['rs'] = 1.0 / dat.Cc
    dat['rh'] = 1.0 / gl_sh
    dat['rr'] = dat.rho_night * Cp / (
            4.0 * SBC * (dat.ta_night + 273.15) * (dat.ta_night + 273.15) * (dat.ta_night + 273.15))
    dat['ra'] = (dat.rh * dat.rr) / (dat.rh + dat.rr)
    dat['LEtrans'] = (
                             dat.s_night * dat.Ac_night + dat.rho_night * Cp * dat.vpd_night * dat.Fc * dat.nightlength / dat.ra) * (
                             1. - dat.Fwet_night) / (dat.s_night + dat.gama_night * (1 + dat.rs / dat.ra))

    df['LEtrans_night'] = dat.LEtrans
    return df


def LEtrans_day_no_scalars(df, Cl, gl_sh, vpd_open, vpd_close, tmin_open, tmin_close, sm_open, sm_close):
    df['ta'] = ((df.ta_day + 273.15) / 293.15) ** 1.75
    dat = df
    dat['rcorr'] = 1.0 / ((101300 / dat.pa) * dat.ta)
    ...
    if sm_open is not None and sm_close is not None:
        dat = mSM(dat, sm_open=sm_open, sm_close=sm_close)

    dat['Gs1'] = Cl * dat.rcorr
    dat['Gcu'] = dat.rcorr * 0.00001
    dat['Gs2'] = gl_sh
    dat['Cc'] = dat.LAI * (1.0 - dat.Fwet_day) * (dat.Gs2 * (dat.Gs1 + dat.Gcu)) / (dat.Gs1 + dat.Gs2 + dat.Gcu)
    ...
    df['LEtrans_day_no_scalars'] = dat.LEtrans
    return df
...
def LEwetsoil_day(df):
    ...
    df['LEwetsoil_day'] = (df.s_day * df.Asoil_day + df.rho_day * Cp * (
            1.0 - df.Fc) * df.vpd_day * df.daylength / df.ras) * df.Fwet_day / (
                                  df.s_day + df.gama_day * df.rtot_day / df.ras)
...
def LEwetsoil_night(df):
    ...
    df['LEwetsoil_night'] = (df.s_night * df.Asoil_night + df.rho_night * Cp * (
            1.0 - df.Fc) * df.vpd_night * df.nightlength / df.ras) * df.Fwet_night / (
                                    df.s_night + df.gama_night * df.rtot_night / df.ras)
...
def PLE_soil_night(df):
    df['ras'] = df.rtot_night * df.rrs_night / (df.rtot_night + df.rrs_night)
    df['PLEsoil_night'] = (df.s_night * df.Asoil_night + df.rho_night * Cp * (
            1.0 - df.Fc) * df.vpd_night * df.nightlength / df.ras) * \
...
def PLE_soil_day(df):
    df['ras'] = df.rtot_day * df.rrs_day / (df.rtot_day + df.rrs_day)
    df['PLEsoil_day'] = (df.s_day * df.Asoil_day + df.rho_day * Cp * (
            1.0 - df.Fc) * df.vpd_day * df.daylength / df.ras) * \
...
# total soil LE
def LEsoil_day(df, beta, sm_open, sm_close):
    df = PLE_soil_day(df)
    df = LEwetsoil_day(df)
    ...
    if sm_open is not None and sm_close is not None:
        ...
        df['fSM_day'] = (df.rh * 0.01) ** (df.vpd_day / beta)
...
# total soil LE
def LEsoil_night(df, beta, sm_open, sm_close):
    ...
    if sm_open is not None and sm_close is not None:
        ...
        df['fSM_night'] = (df.rh * 0.01) ** (df.vpd_night / beta)
...
def MOD16(df, tmin_open, tmin_close, vpd_close, vpd_open, gl_sh,
          gl_e_wv, Cl, rbl_min, rbl_max, beta=None, sm_open=None, sm_close=None, no_scalars=False):
    assert beta or sm_open, 'Either beta or sm_close and sm_open must be included as parameters.'
    assert (beta and not sm_open) or (sm_open and not beta), 'Cannot run the model with both beta and sm parameters.'
    ...
    df = LEsoil_day(df, beta=beta, sm_open=sm_open, sm_close=sm_close)
    df = LEsoil_night(df, beta=beta, sm_open=sm_open, sm_close=sm_close)
    ...
    if no_scalars:
        df = LEtrans_day_no_scalars(df, Cl=Cl, gl_sh=gl_sh, vpd_open=vpd_open, vpd_close=vpd_close,
                                    tmin_open=tmin_open, tmin_close=tmin_close, sm_open=sm_open, sm_close=sm_close)
        df['LE_day_no_scalars'] = df.LEwet_day + df.LEtrans_day_no_scalars + df.LEsoil_day
        df['ET_day_no_scalars'] = df.LE_day_no_scalars / df.lamda_day
        df['simulation_no_scalars'] = df.ET_day_no_scalars + df.ET_night
